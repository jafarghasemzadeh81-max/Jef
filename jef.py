...
"""
═══════════════════════════════════════════════════════════════════════════
         ADVANCED TRADING BOT v18.0 - COMPLETE SELF-EVOLVING MASTER
═══════════════════════════════════════════════════════════════════════════

✅ خودکامل‌گر کامل - تمام پارامترها را خودش تنظیم می‌کند
✅ ML Auto-Pilot - خودش تصمیم می‌گیرد روشن شود یا نه
✅ گزارش کامل هر ۱ ساعت با تمام تغییرات و دلایل
    ...
    def _optimize_risk_parameters(self) -> None:
        try:
            ...
            if winrate > 55 and profit_factor > 1.2 and sharpe > 0.5:
                multiplier = min(1.6, 1.0 + (winrate - 55) / 70 + (profit_factor - 1.2) / 4)
            elif winrate < 45 or profit_factor < 0.85:
            ...
            base_risk = CONFIG.get("RISK_PERCENT", 0.6)
...
def test_telegram_connection() -> bool:
    ...
    try:
        ...
        if r.status_code == 200 and data.get("ok"):
            logger.info(f"✅ اتصال Telegram Bot موفق است")
            return True
        logger.error(f"❌ اتصال Telegram Bot ناموفق | HTTP={r.status_code}")
    except Exception as e:
...
def enhanced_analysis(symbol: str, direction_hint: Optional[str] = None, recovery_mode: bool = False) -> Optional[dict]:
    try:
        ...
        min_confidence = min(hard_cap,
                             float(CONFIG.get("MIN_CONFIDENCE_SCORE", 5.5)) +
        ...
        if idle_min >= starvation_start:
            ...
            recovery = min(0.75, recovery_steps * 0.10)
        ...
        if actual_conf >= min_confidence:
            ...
            _diag_inc("ACCEPTED", f"{regime}|conf={actual_conf:.2f}|required={min_confidence:.2f}", regime, accepted=True)
            return signal
...
# ============================================================================
# بخش 20: گزارش‌سازی کامل - هر ۱ ساعت
# ============================================================================
...
def build_complete_report(price_map: Dict[str, float]) -> str:
    """گزارش کامل ۱ ساعته با تمام تغییرات و دلایل"""
    ...
    try:
        ...
        lines = []
        lines.append("=" * 60)
        lines.append("📊 **گزارش جامع ۱ ساعته - نسخه 19.0 FINAL**")
        ...
        # 2. عملکرد ML
        # ============================
        ml_enabled = CONFIG.get("ML_MAX_INFLUENCE", 0.0) > 0
        lines.append("")
        lines.append("🧠 **وضعیت ML:**")
        lines.append(f"  • وضعیت: {'✅ فعال' if ml_enabled else '❌ غیرفعال'}")
        lines.append(f"  • سهم فعلی: {CONFIG.get('ML_MAX_INFLUENCE', 0.0)*100:.0f}%")
        ...
        for regime, stats in sorted(regime_perf.items()):
            if regime in ("RANGING",):
                continue
            adj = CONFIG.get("REGIME_CONFIDENCE_ADJUSTMENT", {}).get(regime, 0)
            lines.append(f"  • {regime}: {stats['wins']}/{stats['total']} برد ({stats.get('winrate', 0):.0f}%) | آستانه: {5.5 + adj:.1f}")
        ...
        if param_changes:
            ...
            for change in param_changes[-5:]:
                lines.append(f"  • {change.get('parameter', '?')}: {change.get('old', '?')} → {change.get('new', '?')}")
...
# ============================================================================
# بخش 21: حلقه اصلی - نسخه 19.0 FINAL
# ============================================================================
...
def main_loop_v18():
    ...
    while True:
        try:
            ...
            # گزارش ۱ ساعته
            # ============================
            if time.time() - last_report_time >= 3600:  # هر ۱ ساعت
                logger.info("📨 ارسال گزارش ۱ ساعته...")
                report_text = build_complete_report(price_map)
                logger.info("\n" + report_text + "\n")
                
                if CONFIG.get("TELEGRAM_BOT_TOKEN") and CONFIG.get("TELEGRAM_CHAT_ID"):
                    report_sent = send_telegram_message(report_text, message_type="hourly report")
                    if report_sent:
...
# ============================================================================
# بخش 22: نقطه شروع - نسخه 18.0
# ============================================================================

if __name__ == "__main__":
    print("=" * 60)
    print("🧬 **نسخه 19.0 FINAL - COMPLETE SELF-EVOLVING MASTER**")
    print("🤖 کاملاً خودکار - بدون نیاز به هیچ دخالت انسانی")
    print("📊 همه پارامترها خودشان بهینه می‌شوند")
    print("📈 گزارش کامل هر ۱ ساعت با تمام تغییرات و دلایل")
    print("🎯 هدف: دستیابی به بالاترین وین‌ریت ممکن")
    print("=" * 60)
    print()
