...
def calculate_signal_confidence_with_learning(signal_data: dict, current_features: dict) -> float:
    try:
        ...
        pa_signals = signal_data.get("pa_signals", []) or []
        directional_pa = 0
        for pa in pa_signals:
            ...
            if any(x in p for x in ("engulfing صعودی", "higher highs", "higher lows", "bull", "long")):
                ...
                # Inside bars / pin bars are not intrinsically directional here.
                pa_dir = "NEUTRAL"
            if pa_dir == signal_direction:
                directional_pa += 1
            elif pa_dir == "NEUTRAL":
                directional_pa += 0.5
        score += min(directional_pa, 2)
