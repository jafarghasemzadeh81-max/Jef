        ...
        try:
            ...
            with CONFIG_LOCK:
                for key in ["ATR_MULT_SL", "EMA_SHORT", "EMA_MED", "RISK_PERCENT", 
                           ...
                           "MAX_HOLD_MINUTES", "ADX_TREND_MIN", "RSI_PERIOD"]:
...
def enhanced_analysis(symbol: str, direction_hint: Optional[str] = None, recovery_mode: bool = False) -> Optional[dict]:
    try:
        ...
        if idle_min >= starvation_start:
            ...
            recovery = min(0.50, recovery_steps * 0.10)
            min_confidence = max(4.5, min_confidence - recovery)
            min_confidence = min(min_confidence, float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.8)))
...
def build_complete_report(price_map: Dict[str, float]) -> str:
    ...
    try:
        ...
        lines = []
        lines.append("=" * 60)
        lines.append("📊 **گزارش جامع ۱ ساعته - نسخه 18.0**")
...
def main_loop_v18():
    ...
    """
    logger.info("=" * 60)
    logger.info("🧬 **نسخه 18.0 - COMPLETE SELF-EVOLVING MASTER**")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🧬 **نسخه 18.0 - COMPLETE SELF-EVOLVING MASTER**")
