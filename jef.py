...
_TELEGRAM_TOKEN_FALLBACK = "8044605578:AAHUSlnmKQKydHFB2WECwPpZc50gcTflHBA"
...
def get_dynamic_min_confidence(regime: str = "UNKNOWN", recovery_mode: bool = False) -> float:
    """Single source of truth for the confidence threshold used by the final gate."""
    hard_cap = float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.8))
    ...
    if recovery_mode:
        minimum = min(minimum, float(CONFIG.get("FALLBACK_MIN_CONFIDENCE", 5.0)))

    idle_min = max(0.0, (time.time() - _LAST_ACCEPTED_SIGNAL_TS) / 60.0)
    starvation_start = float(CONFIG.get("SIGNAL_STARVATION_MINUTES", 12))
    starvation_enabled = not CONFIG.get("PRO_DISABLE_STARVATION_LOOSENING", True)
    if starvation_enabled and idle_min >= starvation_start:
        ...
        # Before starvation is active, preserve the normal PRO floor.
