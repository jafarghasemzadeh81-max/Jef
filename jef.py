...
CONFIG.update({
    ...
    "ML_MIN_WF_AUC": 0.56,
    "ML_MAX_WF_AUC_STD": 0.10,
    "ML_MIN_WF_FOLDS": 3,
    "ML_REQUIRE_BRIER_MAX": 0.255,
    "ML_ALLOW_PROVISIONAL_MODEL": True,
    "ML_AUTO_HEAL": True,
    ...
    "REGIME_MAX_ADJUSTMENT": 1.0,
    "REGIME_ADJUSTMENT_STEP": 0.10,
    ...
    "MAX_EFFECTIVE_CONFIDENCE": 7.2,
    "SIGNAL_STARVATION_MINUTES": 45,
    "SIGNAL_STARVATION_THRESHOLD_MIN": 1.4,
    "SIGNAL_STARVATION_THRESHOLD_MAX": 2.2,
    "SIGNAL_STARVATION_RECOVERY_STEP": 0.2,
    "AUTO_HEAL_MIN_TRADES": 30,
    "ROLLBACK_IF_PF_DROP": 0.12,
    "ROLLBACK_IF_WR_DROP": 0.08,
...
class MLConfidenceModel:
    ...
    def train(self, force: bool = False) -> Optional[dict]:
        ...
        try:
            ...
            stable = wf_auc >= CONFIG.get("ML_MIN_WF_AUC", 0.56) and wf_std <= CONFIG.get("ML_MAX_WF_AUC_STD", 0.08)
    ...
    def get_influence_weight(self) -> float:
        ...
        if self.wf_auc_std is not None and self.wf_auc_std > CONFIG.get("ML_MAX_WF_AUC_STD", 0.08):
            return 0.0
        if self.last_test_auc is not None and self.last_test_auc < min_auc:
            return 0.0
        if self.last_brier is not None and self.last_brier > CONFIG.get("ML_REQUIRE_BRIER_MAX", 0.26):
            return 0.0
        ...
        return maturity * quality * CONFIG.get("ML_MAX_INFLUENCE", 0.18)
...
class SelfHealingAutoPilot:
    ...
    def regime_adjust(self):
        ...
        for r, tr in regimes.items():
            ...
            if wr < CONFIG["REGIME_BAD_WINRATE"]: cur=min(CONFIG["REGIME_MAX_ADJUSTMENT"],cur+CONFIG["REGIME_ADJUSTMENT_STEP"]); changed=True
            elif wr > CONFIG["REGIME_GOOD_WINRATE"]: cur=max(0.0,cur-CONFIG["REGIME_ADJUSTMENT_STEP"]); changed=True
            adj[r]=round(cur,2)
        ...
        with self.lock:
            ...
            if rm["pf"] < 0.90 or rm["wr"] < 0.45:
                ...
                if last_wr is not None and last_pf is not None and (last_wr-rm["wr"]>=CONFIG["ROLLBACK_IF_WR_DROP"] or last_pf-rm["pf"]>=CONFIG["ROLLBACK_IF_PF_DROP"]):
                    ...
                    CONFIG["MIN_CONFIDENCE_SCORE"]=min(6.8, CONFIG["MIN_CONFIDENCE_SCORE"]+0.15)
...
def enhanced_analysis(symbol: str, direction_hint: Optional[str] = None) -> Optional[dict]:
    ...
    try:
        ...
        min_confidence = min(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 7.2), CONFIG["MIN_CONFIDENCE_SCORE"] + CONFIG.get("REGIME_CONFIDENCE_ADJUSTMENT", {}).get(regime, 0))
...
def build_learning_report(price_map: Dict[str, float]) -> str:
    ...
    try:
        ...
        if not auto_learning.is_healthy():
            lines.append("")
            lines.append("🔴🔴🔴 هشدار: ترد پردازش صف یادگیری متوقف است! معاملات جدید ثبت نمی‌شوند 🔴🔴🔴")
        ...
        # کیفیت زمان‌بندی ورود: Trigger → Entry Timing → TP1 Probability → Time-to-TP
...
def get_adaptive_trigger_thresholds() -> tuple:
    ...
    try:
        ...
        if idle_min >= CONFIG.get("SIGNAL_STARVATION_MINUTES", 45):
            steps = int((idle_min - CONFIG.get("SIGNAL_STARVATION_MINUTES", 45)) // 30) + 1
            base -= steps * CONFIG.get("SIGNAL_STARVATION_RECOVERY_STEP", 0.2)
            base = max(CONFIG.get("SIGNAL_STARVATION_THRESHOLD_MIN", 1.4), base)
        base = min(CONFIG.get("SIGNAL_STARVATION_THRESHOLD_MAX", 2.2), base)
        return base, -base
    except Exception:
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 سیستم تحلیل سیگنال نسخه 12.0 AUTO-PILOT - دیدبانی رد/قبول سیگنال به‌ازای رژیم بازار")
