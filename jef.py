...
"""
═══════════════════════════════════════════════════════════════════════════
         ADVANCED TRADING BOT v19.1 - STABLE SELF-EVOLVING MASTER
═══════════════════════════════════════════════════════════════════════════
...
نسخه 19.1 پایدار - بدون خلاصه‌سازی
"""
...
CONFIG = {
    ...
    "ML_MIN_TRADES_TO_TRAIN": 50,
    "ML_RETRAIN_INTERVAL": 15,
    "ML_FULL_MATURITY_TRADES": 300,
    "ML_MIN_AUC_FOR_INFLUENCE": 0.52,
    ...
    "VOLATILITY_HIGH_THRESHOLD": 0.040,
    ...
    "REPORT_VERSION": "20.0-PRO",
    "PRO_MIN_CONFIDENCE": 6.20,
    "PRO_MIN_RR_TP1": 0.80,
    "PRO_MAX_PRE_SIGNAL_MOVE_PCT": 2.50,
    "PRO_MIN_BACKTEST_WR": 45.0,
    "PRO_MIN_ML_PROBABILITY": 0.55,
    # Edge منفی فقط افزایش ریسک را قفل می‌کند؛ برای جلوگیری از deadlock سیگنال،
    # آستانه ورود جدا و قابل دستیابی است.
    "PRO_NEGATIVE_EDGE_MIN_CONFIDENCE": 6.20,
    ...
    "SYMBOL_COOLDOWN_SEC": 1500,
    "MAX_PENDING_PER_SYMBOL": 1,
    "AUTO_LEARN_QUEUE_MAX": 500,
    "MAX_NEW_SIGNALS_PER_CYCLE": 4,
    "MAX_NEW_SIGNALS_PER_HOUR": 25,
    ...
    "SIGNAL_STARVATION_RECOVERY_STEP": 0.10,
    ...
    "FALLBACK_SCAN_AFTER_MINUTES": 8,
    "FALLBACK_CANDIDATES": 12,
    "FALLBACK_MIN_CONFIDENCE": 5.0,
    # Recovery mode: after prolonged signal starvation, relax only the
    # non-safety filters enough to prevent a permanent zero-signal state.
    "RECOVERY_RELAX_VOLATILITY": True,
    "RECOVERY_ALLOW_RANGING": True,
    "RECOVERY_MIN_CONFIDENCE": 5.0,
    "RECOVERY_MAX_PRE_SIGNAL_MOVE_PCT": 3.0,
}

BASE_CONFIG = CONFIG.copy()

# === TELEGRAM FIX: مقادیر نهایی و قطعی ===
# Environment فقط در صورت وجود مقدار معتبر override می‌کند.
_TELEGRAM_TOKEN_FALLBACK = "8044605578:AAES3EaT-3pbYg9cO1R59agEfg4TLQ1-3HA"
_TELEGRAM_CHAT_ID_FALLBACK = "-1002906437733"

_env_token = os.environ.get("TELEGRAM_BOT_TOKEN", "").strip()
_env_chat_id = os.environ.get("TELEGRAM_CHAT_ID", "").strip()

# The token/chat ID below are authoritative for this deployment.
# This prevents an old Railway variable from overriding the new token.
CONFIG["TELEGRAM_BOT_TOKEN"] = _TELEGRAM_TOKEN_FALLBACK
CONFIG["TELEGRAM_CHAT_ID"] = _TELEGRAM_CHAT_ID_FALLBACK
# =========================================
# امنیت: توکن تلگرام فقط از Environment خوانده می‌شود؛ هرگز داخل سورس ذخیره نکنید.
# Use the newly issued token as the default. An explicitly set environment
# variable still overrides it, which is useful for deployment secrets.
CONFIG["TELEGRAM_BOT_TOKEN"] = os.environ.get("TELEGRAM_BOT_TOKEN", "").strip() or _TELEGRAM_TOKEN_FALLBACK
CONFIG["TELEGRAM_CHAT_ID"] = os.environ.get("TELEGRAM_CHAT_ID", "").strip() or _TELEGRAM_CHAT_ID_FALLBACK
    ...
    def add_trade_result(self, trade_data: dict) -> None:
        try:
            trade_record = {
                ...
                # Preserve the simulator's resolved outcome when available; do not
                # silently recompute win from net return and create label conflicts.
                "win": bool(trade_data.get("win", trade_data.get("return_pct", 0.0) > 0)),
                ...
                "risk_percent_used": trade_data.get("risk_percent_used", CONFIG.get("RISK_PERCENT", 0.0)),
...
class SelfHealingAutoPilot:
    ...
    def symbol_blocked(self, symbol, direction):
        """Block duplicate/open exposure using both resolved trades and pending learning items."""
        now = time.time()
        window = float(CONFIG.get("DEDUP_WINDOW_SEC", 1500))
        wanted_direction = "LONG" if "LONG" in str(direction).upper() else "SHORT" if "SHORT" in str(direction).upper() else str(direction).upper()
        recent = [
            t for t in list(self.memory.trades)
            if t.get("symbol") == symbol and
            ("LONG" if "LONG" in str(t.get("direction", "")).upper() else "SHORT" if "SHORT" in str(t.get("direction", "")).upper() else str(t.get("direction", "")).upper()) == wanted_direction and
            now - float(t.get("timestamp", 0)) < window
        ]
        pending = []
        try:
            pending = [
                x for x in list(auto_learning.learning_queue)
                if x.get("symbol") == symbol and
                ("LONG" if "LONG" in str(x.get("direction", "")).upper() else "SHORT" if "SHORT" in str(x.get("direction", "")).upper() else str(x.get("direction", "")).upper()) == wanted_direction
            ]
        except Exception:
            pending = []
        # Enforce the configured post-loss cooldown as well.
        last = [t for t in list(self.memory.trades) if t.get("symbol") == symbol]
        if last:
            last_trade = max(last, key=lambda t: float(t.get("timestamp", 0)))
            if not bool(last_trade.get("win", False)):
                if now - float(last_trade.get("timestamp", 0)) < float(CONFIG.get("COOLDOWN_AFTER_LOSS", 0)):
                    return True
        return (len(recent) + len(pending)) >= int(CONFIG.get("MAX_PENDING_PER_SYMBOL", 1))
...
class AutoLearningSystem:
    def __init__(self, memory: PerformanceMemory):
        self.memory = memory
        self.learning_queue: deque = deque(maxlen=int(CONFIG.get("AUTO_LEARN_QUEUE_MAX", 500)))
    ...
    def _simulate_outcome(self, signal_data: dict):
        try:
            ...
            trade_result = {
                ...
                "risk_percent_used": signal.get("risk_percent_used", CONFIG.get("RISK_PERCENT", 0.0)),
            }
            ...
            try:
                ...
                while len(self.learning_queue) > 0:
                    ...
                    if age >= hard_expiry_seconds:
                        # Preserve the item for an explicit final unresolved record instead of silently dropping learning data.
                        self.learning_queue.popleft()
                        unresolved = {
                            "symbol": signal_data.get("symbol"),
                            "direction": signal_data.get("direction"),
                            "entry": signal_data.get("entry"),
                            "exit": signal_data.get("entry"),
                            "return_pct": 0.0,
                            "win": False,
                            "which_target": "UNRESOLVED",
                            "outcome_label": "UNRESOLVED",
                            "time_to_result_min": age / 60.0,
                            "features": (signal_data.get("signal") or {}).get("learning_features", {}),
                            "confidence": signal_data.get("confidence", 0),
                            "auto_learned": True,
                            "risk_percent_used": (signal_data.get("signal") or {}).get("risk_percent_used", CONFIG.get("RISK_PERCENT", 0.0)),
                        }
                        self.memory.add_trade_result(unresolved)
                        logger.error(f"🔴 {signal_data.get('symbol')} پس از {age/60:.0f} دقیقه unresolved ثبت شد")
                        continue
...
def get_max_hold_candles() -> int:
    minutes_per_candle = _interval_to_minutes(CONFIG.get("KLINE_INTERVAL", "15m")) or 15
    # Never understate the requested wall-clock holding horizon.
    return max(1, int(math.ceil(float(CONFIG.get("MAX_HOLD_MINUTES", 45)) / minutes_per_candle)))

def send_telegram_message(text: str, message_type: str = "message", retries: int = 3) -> bool:
    token = (_TELEGRAM_TOKEN_FALLBACK or CONFIG.get("TELEGRAM_BOT_TOKEN") or _env_token).strip()
    chat_id = (_TELEGRAM_CHAT_ID_FALLBACK or CONFIG.get("TELEGRAM_CHAT_ID") or _env_chat_id).strip()
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if response.status_code == 200 and body.get("ok") is True:
                logger.debug(f"✅ Telegram {message_type} ارسال شد")
                return True
            description = body.get("description", response.text[:300])
            logger.warning(f"⚠️ Telegram {message_type} ارسال نشد | HTTP={response.status_code} | {description}")
...
def test_telegram_connection() -> bool:
    ...
    try:
        ...
        if r.status_code == 200 and data.get("ok"):
            ...
            test = send_telegram_message("🟢 Cezartrading 20 PRO\n✅ اتصال Telegram با موفقیت تست شد.", message_type="startup test", retries=2)
            if test:
                logger.info(f"✅ پیام تست به Chat ID {chat_id} ارسال شد")
                return True
            logger.error("❌ Token معتبر است اما ارسال به Chat ID انجام نشد؛ عضویت/مجوز ربات و Chat ID را بررسی کن")
            return False
    ...
    try:
        ...
        for d in data:
            ...
            ret.append({
                "symbol": sym.upper(),
                "current_price": last,
                # Binance /fapi/v1/ticker/24hr returns a 24h change, not 1h.
    ...
    try:
        ...
        for attempt in range(3):
            try:
                ...
                # Binance's last kline is normally still forming. When requested,
                # remove it so all indicators/entries/backtests use closed candles only.
                if df is not None and not df.empty and CONFIG.get("REQUIRE_CLOSED_CANDLE", True):
                    if len(df) > 1:
                        df = df.iloc[:-1].copy()
                    else:
                        df = pd.DataFrame()
...
def vwap_pd(high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series) -> pd.Series:
    ...
    # Do not back-fill VWAP with future observations.
    return (cum_tp_vol / cum_vol).ffill().fillna(close)
...
def compute_confirmation_signals(df: pd.DataFrame, require_volume: bool = True) -> pd.DataFrame:
    try:
        ...
        idx = getattr(df, "index", pd.RangeIndex(1))
        return pd.DataFrame({
            "long_confirm": pd.Series(False, index=idx),
            "short_confirm": pd.Series(False, index=idx),
            "adx": pd.Series(np.nan, index=idx),
            "cci": pd.Series(np.nan, index=idx),
            "williams_r": pd.Series(np.nan, index=idx),
            "mfi": pd.Series(np.nan, index=idx),
            "supertrend_dir": pd.Series(0, index=idx),
        })
...
def backtest_symbol_realistic(symbol: str, candles: int = None) -> dict:
    try:
        ...
        for i in range(min_idx, n - 1):
            ...
            if result is None:
                # Timeout is resolved by the actual directional return at the hold horizon,
                # matching the auto-learning outcome logic instead of forcing every timeout to a loss.
                exit_price = df.loc[min(entry_idx + max_hold, n - 1), "close"]
                result = "win" if ((float(exit_price) > float(entry_price)) == (signal_direction == "LONG")) else "loss"
...
def _directional_price_action_quality(pa_signals, direction: str) -> float:
    """Return directional PA quality in [0, 1]. Neutral patterns contribute half.
    Opposite-direction patterns do not increase the score.
    """
    try:
        target = "LONG" if "LONG" in str(direction).upper() else "SHORT" if "SHORT" in str(direction).upper() else ""
        if not target:
            return 0.0
        total = 0.0
        count = 0
        for pa in (pa_signals or []):
            p = str(pa).lower()
            if any(x in p for x in ("engulfing صعودی", "higher highs", "higher lows", "bull", "long")):
                pa_dir = "LONG"
            elif any(x in p for x in ("engulfing نزولی", "lower highs", "lower lows", "bear", "short")):
                pa_dir = "SHORT"
            else:
                pa_dir = "NEUTRAL"
            count += 1
        ...
        if count == 0:
            return 0.0
        return min(1.0, total / min(count, 3))
    except Exception:
        return 0.0
...
def calculate_signal_confidence_with_learning(signal_data: dict, current_features: dict) -> float:
    try:
        ...
        trend = str(signal_data.get("trend", "")).upper()
        signal_direction = "LONG" if "LONG" in str(signal_data.get("direction", "")).upper() else "SHORT" if "SHORT" in str(signal_data.get("direction", "")).upper() else ""
        trend_direction = "LONG" if "LONG" in trend else "SHORT" if "SHORT" in trend else ""
        trend_aligned = bool(signal_direction and trend_direction and signal_direction == trend_direction)
        if trend_aligned and trend.startswith("STRONG "):
            score += 3
        elif trend_aligned and trend in ("LONG", "SHORT"):
            score += 2
        elif trend == "NEUTRAL":
            score += 1
        ...
        pa_quality = _directional_price_action_quality(signal_data.get("pa_signals", []) or [], signal_direction)
        score += pa_quality * 2.0
...
def enhance_signal_with_learning(signal: dict, df: pd.DataFrame) -> dict:
    ...
    try:
        signal_dir = "LONG" if "LONG" in str(signal.get("direction", "")).upper() else "SHORT" if "SHORT" in str(signal.get("direction", "")).upper() else ""
        trend_text = str(signal.get("trend", "")).upper()
        trend_dir = "LONG" if "LONG" in trend_text else "SHORT" if "SHORT" in trend_text else ""
        trend_alignment = 1.0 if signal_dir and trend_dir == signal_dir else 0.0
        if signal_dir and trend_dir == signal_dir and trend_text.startswith("STRONG "):
            trend_alignment = 1.0
        ...
        current_features = {
            ...
            "price_action_quality": _directional_price_action_quality(signal.get("pa_signals", []), signal_dir),
        ...
        if df is not None and len(df) > 0:
            if "rsi" in df.columns:
                ...
                # Preserve direction: positive = bullish RSI, negative = bearish RSI.
                current_features["rsi_momentum"] = (current_features["rsi"] - 50.0) / 50.0
...
def analyze_symbol_full(symbol: str, direction_hint: Optional[str] = None, use_multi_tf: bool = True) -> Optional[dict]:
    try:
        ...
        # Keep the OHLCV dataframe for risk management. create_ml_features() may
        # return a feature-only dataframe without the original `close` column.
        # Therefore risk must be calculated from the original market dataframe
        # after ATR has been computed.
        ...
        df["atr"] = atr_pd(df["high"], df["low"], df["close"], CONFIG["ATR_PERIOD"])
        adaptive_risk = dynamic_risk_management(symbol, df)
        df["atr_ratio"] = df["atr"] / df["close"].replace(0, np.nan)
        df["rsi"] = rsi_pd(df["close"], CONFIG["RSI_PERIOD"])
        df["volume_ratio"] = df["volume"] / df["volume"].shift(1).rolling(20, min_periods=5).mean()
        ...
        signal = {
            ...
            "risk_percent_used": float(adaptive_risk),
            "swings": swings,
...
def _diag_inc(stage: str, reason: str = "", regime: str = "UNKNOWN", accepted: bool = False):
    ...
    try:
        ...
        with _SIGNAL_DIAG_LOCK:
            ...
            for bucket_name in ("cumulative", "hourly"):
                bucket = _SIGNAL_DIAG_STATE.setdefault(bucket_name, {})
                rec = bucket.setdefault(key, {"count": 0, "stage": stage, "reason": reason or "ok"})
                rec["count"] = int(rec.get("count", 0)) + 1
            if accepted:
                _SIGNAL_DIAG_STATE.setdefault("hourly", {}).setdefault("ACCEPTED:ok", {"count": 0, "stage": "ACCEPTED", "reason": "ok"})["count"] += 1
...
def get_dynamic_min_confidence(regime: str = "UNKNOWN", recovery_mode: bool = False) -> float:
    """Single confidence threshold source. Recovery can lower entry threshold safely."""
    hard_cap = float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.6))
    base = float(CONFIG.get("MIN_CONFIDENCE_SCORE", 5.5))
    adj = float(CONFIG.get("REGIME_CONFIDENCE_ADJUSTMENT", {}).get(regime, 0.0))
    minimum = min(hard_cap, base + adj)

    idle_min = max(0.0, (time.time() - _LAST_ACCEPTED_SIGNAL_TS) / 60.0)
    starvation_start = float(CONFIG.get("SIGNAL_STARVATION_MINUTES", 12))
    starvation_enabled = not bool(CONFIG.get("PRO_DISABLE_STARVATION_LOOSENING", False))

    if recovery_mode:
        # Explicit recovery floor; do not let PRO_MIN_CONFIDENCE re-tighten it.
        minimum = min(minimum, float(CONFIG.get("RECOVERY_MIN_CONFIDENCE", 5.0)))
    elif starvation_enabled and idle_min >= starvation_start:
        steps = int((idle_min - starvation_start) // 15) + 1
        recovery = min(0.75, steps * float(CONFIG.get("SIGNAL_STARVATION_RECOVERY_STEP", 0.10)))
        minimum = max(4.5, minimum - recovery)
    else:
        minimum = max(minimum, float(CONFIG.get("PRO_MIN_CONFIDENCE", 6.2)))

    return min(minimum, hard_cap)
...
def pro_signal_quality_gate(signal: dict) -> Tuple[bool, str, dict]:
    ...
    try:
        ...
        regime = str(signal.get("market_regime", signal.get("regime", "UNKNOWN")))
        min_conf = get_dynamic_min_confidence(regime, bool(signal.get("recovery_mode", False)))
        diagnostics["required_confidence"] = min_conf
        signal["accepted_min_confidence"] = min_conf
        if conf < min_conf:
            return False, f"pro_confidence<{min_conf:.2f}", diagnostics
        ...
        if pre_move is not None:
            ...
            max_pre_move = (float(CONFIG.get("RECOVERY_MAX_PRE_SIGNAL_MOVE_PCT", 3.0))
                            if bool(signal.get("recovery_mode", False))
                            else float(CONFIG.get("PRO_MAX_PRE_SIGNAL_MOVE_PCT", 2.5)))
            if pre_move_abs > max_pre_move:
        ...
        if signal.get("ml_used") and ml_p is not None:
            ...
            # ML is a bounded confidence input, not a 100% veto.
            # A model with <=8% influence must not be able to erase the other evidence.
            diagnostics["ml_gate"] = "shadow_only"
            if ml_p < float(CONFIG.get("PRO_MIN_ML_PROBABILITY", 0.55)):
                diagnostics["ml_weak"] = True

        # وقتی سیستم در مجموع Edge منفی دارد، فقط سیگنال‌های قوی‌تر اجازه عبور دارند.
        ...
        if pf30 < 0.85 or exp30 < 0:
            diagnostics["edge_state"] = "NEGATIVE"
            # مهم: قبلاً اینجا conf < 6.8 بود، در حالی که
            # MAX_EFFECTIVE_CONFIDENCE در همین نسخه 6.6 است؛ بنابراین
            # وقتی Edge منفی بود هیچ سیگنالی هرگز نمی‌توانست عبور کند.
            # Edge منفی باید افزایش RISK را قفل کند، نه اینکه مسیر ورود را deadlock کند.
            if bool(signal.get("recovery_mode", False)):
                # Negative edge locks risk, not signal generation. In recovery
                # mode require the same controlled recovery floor as entry.
                neg_edge_min = float(CONFIG.get("RECOVERY_MIN_CONFIDENCE", 5.0))
            else:
                neg_edge_min = float(CONFIG.get("PRO_NEGATIVE_EDGE_MIN_CONFIDENCE", CONFIG.get("PRO_MIN_CONFIDENCE", 6.2)))
            hard_cap = float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.6))
            neg_edge_min = min(neg_edge_min, hard_cap)
            diagnostics["negative_edge_min_confidence"] = neg_edge_min
            if conf < neg_edge_min:
                return False, f"negative_edge_confidence<{neg_edge_min:.2f}", diagnostics
        else:
            diagnostics["edge_state"] = "OK"
...
def enhanced_analysis(symbol: str, direction_hint: Optional[str] = None, recovery_mode: bool = False) -> Optional[dict]:
    try:
        ...
        volatility_ok = volatility_filter(df)
        if not volatility_ok and not (recovery_mode and CONFIG.get("RECOVERY_RELAX_VOLATILITY", True)):
        ...
        if volatility_ok:
            ...
            _diag_inc("VOLATILITY_OK", "recovery_relaxed")
        ...
        if regime == "RANGING" and not (recovery_mode and CONFIG.get("RECOVERY_ALLOW_RANGING", True)):
            _diag_inc("REJECT", "ranging", regime)
            logger.debug(f"{symbol}: رد شد (بازار رنج)")
            return None
        if regime == "RANGING":
            _diag_inc("REGIME_OK", "recovery_ranging", regime)
        else:
        ...
        # Confidence is evaluated once, by the final PRO quality gate.
        # Keeping only one threshold here removes the previous double-gate mismatch.
        min_confidence = get_dynamic_min_confidence(regime, recovery_mode)
        signal["analysis_required_confidence"] = min_confidence
        signal["recovery_mode"] = bool(recovery_mode)
        return signal
...
def create_ml_features(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        vol_sma_20 = df["volume"].shift(1).rolling(20, min_periods=5).mean()
        ...
        # Never back-fill time-series features from the future. Forward-fill only,
        # then use neutral/zero values for the unavoidable warm-up prefix.
...
def build_complete_report(price_map: Dict[str, float]) -> str:
    ...
    try:
        ...
        lines = []
        lines.append("=" * 60)
        lines.append("📊 **گزارش جامع ۳۰ دقیقه‌ای - نسخه 20.0 PRO**")
        ...
        try:
            ...
            lines.append(f"  • این ساعت: {len(recent_hour_trades)} معامله بسته‌شده | WR {((hour_wins/len(recent_hour_trades))*100 if recent_hour_trades else 0):.1f}% | PF {hour_pf:.2f}")
        except Exception:
        ...
        # ============================
        diag = _diag_snapshot()
        hourly_diag = diag.get("hourly", {})
        if hourly_diag:
            lines.append("")
            lines.append("🧭 **تشخیص مسیر سیگنال در این ساعت:**")
            stage_order = ["TRIGGER", "ANALYSIS_ENTERED", "KLINES_OK", "QUALITY_OK", "VOLATILITY_OK", "BREAKOUT_OK", "REGIME_OK", "ANALYSIS_OK", "GATE_EVALUATED", "ACCEPTED"]
            shown = []
        ...
        # ============================
        # 13. CEZARTRADING 20 PRO diagnostics
        # ============================
...
def _daily_realized_risk_percent() -> float:
    """Sum configured risk for today's resolved/pending signals to enforce the daily cap."""
    try:
        start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
        total = 0.0
        for t in list(performance_memory.trades):
            ts = float(t.get("timestamp", 0))
            if ts >= start:
                total += abs(_safe_float(t.get("risk_percent_used", t.get("risk_percent", CONFIG.get("RISK_PERCENT", 0.0))), 0.0))
        for item in list(auto_learning.learning_queue):
            ts = float(item.get("timestamp", 0))
            if ts >= start:
                sig = item.get("signal", {}) or {}
                total += abs(_safe_float(sig.get("risk_percent_used", CONFIG.get("RISK_PERCENT", 0.0)), 0.0))
        return total
    except Exception:
        return 0.0

def _daily_risk_limit_reached(next_risk_percent: float = None) -> bool:
    try:
        cap = float(CONFIG.get("MAX_RISK_PER_DAY", 10.0))
        nxt = float(next_risk_percent if next_risk_percent is not None else CONFIG.get("RISK_PERCENT", 0.0))
        return _daily_realized_risk_percent() + max(0.0, nxt) > cap
    except Exception:
        return True

def _same_direction_hourly_ratio(direction: str) -> float:
    try:
        hour = int(time.time() // 3600)
        start = hour * 3600
        items = [t for t in list(performance_memory.trades) if float(t.get("timestamp", 0)) >= start]
        items += [x.get("signal", {}) for x in list(auto_learning.learning_queue) if float(x.get("timestamp", 0)) >= start]
        if not items:
            return 0.0
        target = "LONG" if "LONG" in str(direction).upper() else "SHORT"
        same = sum(1 for x in items if target in str(x.get("direction", "")).upper())
        return same / max(1, len(items))
    except Exception:
        return 1.0
...
def main_loop_v18():
    ...
    """
    logger.info("=" * 60)
    logger.info("🧬 **نسخه 20.0 PRO - ADAPTIVE QUALITY + RISK CONTROL**")
    ...
    while True:
        try:
            ...
            for coin in coins:
                ...
                signal_direction = "LONG" if direction_hint == "bull" else "SHORT"
                if self_healing_autopilot.symbol_blocked(symbol, signal_direction):
                    _diag_inc("REJECT", "symbol_pending_or_cooldown")
                    continue
                if _same_direction_hourly_ratio(signal_direction) >= float(CONFIG.get("MAX_SAME_DIRECTION_RATIO", 0.70)):
                    _diag_inc("REJECT", "same_direction_ratio")
                    continue
                if _daily_risk_limit_reached():
                    _diag_inc("REJECT", "daily_risk_limit")
                    continue
                ...
                if summary:
                    ...
                    if not ok_pro:
                        ...
                        logger.debug(f"{symbol}: PRO_GATE رد کرد | {pro_reason} | conf={pro_diag.get('confidence', 0):.2f} | required={pro_diag.get('required_confidence', 'n/a')}")
                        continue
                    signals.append(summary)
                    globals()["_LAST_ACCEPTED_SIGNAL_TS"] = time.time()
                    _register_accepted_signal()
                    _diag_inc("ACCEPTED", "ok", str(summary.get("market_regime", "UNKNOWN")), accepted=True)
                    ...
                    if CONFIG.get("TELEGRAM_BOT_TOKEN"):
                        ...
                        line = f"{summary['symbol']}{regime_tag}{ml_tag}\n\n"
                        line += f"📊 {summary['direction']}\n\n"
                        line += f"🎯 ENTRY: {summary['entry']}\n"
                        line += f"🛑 SL: {summary['sl']}\n"
            ...
            if len(signals) < 2 and idle_min >= fallback_after:
                ...
                for _, ch, symbol in candidates:
                    ...
                    if self_healing_autopilot.symbol_blocked(symbol, direction_text):
                        _diag_inc("REJECT", "symbol_pending_or_cooldown")
                        continue
                    if _same_direction_hourly_ratio(direction_text) >= float(CONFIG.get("MAX_SAME_DIRECTION_RATIO", 0.70)):
                        _diag_inc("REJECT", "same_direction_ratio")
                        continue
                    if _daily_risk_limit_reached():
                        _diag_inc("REJECT", "daily_risk_limit")
                    ...
                    # Fallback signals must use the exact same Telegram path
                    # as normal signals; previously they were accepted/saved
                    ...
                    if CONFIG.get("TELEGRAM_BOT_TOKEN") and CONFIG.get("TELEGRAM_CHAT_ID"):
                        ...
                        for i, tp in enumerate(summary.get('tps', [])[:3]):
                            line += f"🎯 TP{i+1}: {tp}\n"
                        line += f"🎓 اطمینان: {summary.get('confidence', 0):.1f}/10\n"
                        line += "🆘 Recovery mode: فعال"
                        send_telegram_message_with_cooldown(symbol, summary['direction'], line)

            if not signals:
                logger.debug(
                    f"Signal cycle ended with 0 accepted | triggers={pump_count + dump_count} | "
                    f"fallback={'eligible' if idle_min >= float(CONFIG.get('FALLBACK_SCAN_AFTER_MINUTES', 8)) else 'waiting'}"
                )
            ...
            # گزارش ۳۰ دقیقه‌ای
            # ============================
            if time.time() - last_report_time >= 1800:  # هر ۳۰ دقیقه
                logger.info("📨 ارسال گزارش ۳۰ دقیقه‌ای...")
                report_text = build_complete_report(price_map)
                logger.info("\n" + report_text + "\n")
                
                if CONFIG.get("TELEGRAM_BOT_TOKEN") and CONFIG.get("TELEGRAM_CHAT_ID"):
                    report_sent = send_telegram_message(report_text, message_type="30-minute report")
                    if report_sent:
                        last_report_time = time.time()
                    else:
                        logger.warning("⚠️ گزارش ارسال نشد - تلاش مجدد در چرخه بعد")
                else:
                    last_report_time = time.time()  # حتی اگر تلگرام تنظیم نباشد، گزارش را ثبت کن
...
if __name__ == "__main__":
    print("=" * 60)
    print("🧬 **نسخه 20.0 PRO - ADAPTIVE QUALITY + RISK CONTROL**")
