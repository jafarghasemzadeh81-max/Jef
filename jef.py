...
CONFIG = {
    ...
    "PRO_DISABLE_STARVATION_LOOSENING": True,
...
def pro_signal_quality_gate(signal: dict) -> Tuple[bool, str, dict]:
    ...
    try:
        ...
        if conf < float(CONFIG.get("PRO_MIN_CONFIDENCE", 6.2)):
            return False, f"pro_confidence<{CONFIG.get('PRO_MIN_CONFIDENCE', 6.2):.2f}", diagnostics
...
def enhanced_analysis(symbol: str, direction_hint: Optional[str] = None, recovery_mode: bool = False) -> Optional[dict]:
    try:
        ...
        # در نسخه PRO، کمبود سیگنال باعث پایین آوردن کیفیت نمی‌شود.
        # Starvation فقط برای گزارش‌گیری است؛ کیفیت قربانی تعداد سیگنال نمی‌شود.
        if CONFIG.get("PRO_DISABLE_STARVATION_LOOSENING", True):
            min_confidence = max(min_confidence, float(CONFIG.get("PRO_MIN_CONFIDENCE", 6.2)))
