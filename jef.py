...
CONFIG = {
    ...
    "OUTCOME_REPORT_INTERVAL_SEC": 7200,
...
def build_2h_outcome_report(window_sec: Optional[int] = None) -> str:
    """خلاصه معاملات بسته‌شده در بازه (پیش‌فرض ۲ ساعت) + صف هنوز باز."""
    window_sec = int(window_sec or CONFIG.get("OUTCOME_REPORT_INTERVAL_SEC", 7200))
    now = time.time()
    cutoff = now - window_sec
    hours = max(1, int(round(window_sec / 3600)))

    trades = []
    try:
        all_trades = list(getattr(performance_memory, "trades", []) or [])
        trades = [t for t in all_trades if _parse_trade_ts(t) >= cutoff]
    except Exception as e:
        logger.warning(f"2h outcome: read trades failed: {e}")

    lines_detail = []
    n_tp1 = n_tp2 = n_tp3 = n_sl = n_timeout = n_other = 0
    n_win = n_loss = 0
    ...
    for t in trades:
        ...
        lines_detail.append(f"{emoji} {sym} {direction} → {tag}{ret_s}")

    closed = len(trades)
    decided = n_win + n_loss  # بدون TIMEOUT/OTHER
    wr_decided = (n_win / decided * 100.0) if decided > 0 else 0.0
    wr_all = (n_win / closed * 100.0) if closed > 0 else 0.0

    # هنوز باز در صف auto-learn
    pending_lines = []
    pending_n = 0
    try:
        for item in list(getattr(auto_learning, "learning_queue", []) or []):
            sig = item.get("signal", item) if isinstance(item, dict) else {}
            sym = sig.get("symbol") or item.get("symbol") or "?"
            direction = (sig.get("direction") or item.get("direction") or "?").upper()
            pending_lines.append(f"⏳ {sym} {direction} → OPEN")
            pending_n += 1
    except Exception:
        pass

    start_txt = time.strftime("%H:%M", time.localtime(cutoff))
    end_txt = time.strftime("%H:%M", time.localtime(now))
    date_txt = time.strftime("%Y-%m-%d", time.localtime(now))

    lines = [
        f"📋 نتایج {hours} ساعته — {date_txt} {start_txt}–{end_txt}",
        "",
    ]
    if lines_detail:
        lines.extend(lines_detail)
    else:
        lines.append("• هیچ معامله بسته‌شده‌ای در این بازه ثبت نشده.")

    if pending_lines:
        lines.append("")
        lines.append(f"⏳ هنوز باز ({pending_n}):")
        # سقف برای جلوگیری از پیام خیلی بلند
        for row in pending_lines[:40]:
            lines.append(row)
        if pending_n > 40:
            lines.append(f"… و {pending_n - 40} مورد دیگر")

    lines.extend([
        "",
        "📊 جمع:",
        f"بسته شده: {closed} | برد (TP1/2/3): {n_win} | باخت (SL): {n_loss}",
        f"TP1: {n_tp1} | TP2: {n_tp2} | TP3: {n_tp3} | SL: {n_sl} | TIMEOUT: {n_timeout}"
        + (f" | OTHER: {n_other}" if n_other else ""),
        f"موفقیت (بدون TIMEOUT): {wr_decided:.1f}%  ({n_win}/{decided})" if decided else "موفقیت (بدون TIMEOUT): —",
        f"موفقیت روی همه بسته‌شده‌ها: {wr_all:.1f}%  ({n_win}/{closed})" if closed else "موفقیت روی همه بسته‌شده‌ها: —",
        f"صف باز: {pending_n}",
    ])
    return "\n".join(lines)
...
def send_2h_outcome_report_safely() -> bool:
    ...
    try:
        ...
        for i, chunk in enumerate(chunks, 1):
            prefix = f"📋 خلاصه outcome — بخش {i}/{total}\n\n" if total > 1 else ""
...
def main_loop_v18():
    ...
    while True:
        ...
        try:
            ...
            # ============================
            # خلاصه outcome هر ۲ ساعت → کانال گزارش
            # ============================
