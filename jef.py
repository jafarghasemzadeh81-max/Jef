...
def _ensure_signal_font_file() -> Optional[str]:
    """مسیر یک TTF Bold واقعی. اگر روی سیستم نبود از اینترنت یک‌بار دانلود می‌شود."""
    global _SIGNAL_FONT_PATH
    if _SIGNAL_FONT_PATH and os.path.exists(_SIGNAL_FONT_PATH):
            ...
            try:
                ...
                if r.status_code == 200 and len(r.content) > 10000:
                    with open(dest, "wb") as f:
                        f.write(r.content)
                    _SIGNAL_FONT_PATH = dest
                    logger.info(f"✅ Signal font downloaded: {dest} ({len(r.content)} bytes)")
                    return dest
            except Exception as e:
                logger.warning(f"font download failed from {url}: {e}")
        logger.error("❌ No TrueType font available — signal card text will be unreadable")
        return None
...
def send_telegram_photo(photo_path: str, caption: str = "", retries: int = 2) -> bool:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code == 200 and body.get("ok") is True:
                return True
            logger.error(f"❌ Telegram photo HTTP={r.status_code} | {body.get('description', r.text[:300])}")
            if r.status_code in (400, 401, 403):
                return False
        except Exception as e:
            logger.warning(f"⚠️ Telegram photo attempt {attempt}: {e}")
        if attempt < retries:
            time.sleep(2 * attempt)
    return False
