...
def _draw_text_bold(d, xy, text, font, fill):
    """متن ضخیم‌تر برای خوانایی بعد از فشرده‌سازی تلگرام."""
    x, y = xy
    # سایه تیره دور متن
...
def render_signal_gif(summary: dict, out_path: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        # پالت ثابت برای جلوگیری از محو شدن متن در GIF
        base = frames[0].convert("P", palette=Image.ADAPTIVE, colors=128)
        rest = [fr.convert("P", palette=Image.ADAPTIVE, colors=128) for fr in frames[1:]]
        base.save(
            out_path,
            save_all=True,
            append_images=rest,
