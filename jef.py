...
"""
═══════════════════════════════════════════════════════════════════════════
         ADVANCED TRADING BOT v14.0 - COMPLETE SELF-HEALING VERSION
═══════════════════════════════════════════════════════════════════════════
...
✅ بک‌تست واقعی
✅ بهینه‌سازی شده برای عملکرد بهتر
...
CONFIG = {
    ...
    "WEIGHT_SHARPEN_EXPONENT_MAX": 1.8,
    "WEIGHT_SHARPEN_MIN_TRADES": 80,
    "WEIGHT_SHARPEN_FULL_TRADES": 250,

    "ML_MIN_TRADES_TO_TRAIN": 40,
    "ML_RETRAIN_INTERVAL": 15,
    "ML_MAX_INFLUENCE": 0.15,
    "ML_FULL_MATURITY_TRADES": 300,
    "ML_MIN_AUC_FOR_INFLUENCE": 0.50,
    "ML_FULL_AUC_FOR_INFLUENCE": 0.60,
    "ML_MIN_WF_FOLDS": 3,
    "ML_MIN_WF_AUC": 0.50,
    ...
    "REGIME_MIN_TRADES_FOR_ADJUST": 15,
    "REGIME_MAX_ADJUSTMENT": 1.5,
    "REGIME_ADJUSTMENT_STEP": 0.15,
    "REGIME_ADJUSTMENT_SENSITIVITY": 0.10,
    
    "ADAPTIVE_PUMP_THRESHOLD": True,
    "PUMP_THRESHOLD_BASE": 1.2,
    "PUMP_THRESHOLD_MIN": 0.8,
    "PUMP_THRESHOLD_MAX": 2.0,
    ...
    "WF_TRAIN_RATIO": 0.7,
    "LEARNING_WINDOW": 80,
    "ADAPTIVE_UPDATE_INTERVAL": 12,
    "MIN_TRADES_FOR_LEARNING": 15,
    "MIN_TRADES_FOR_CORRELATION": 10,
    "LEARNING_RATE": 0.15,
    "MIN_WF_TEST_TRADES": 5,
    "LEARNING_REPORT_INTERVAL_HOURS": 1,
    ...
    "REGIME_BAD_WINRATE": 0.45,
    "REGIME_GOOD_WINRATE": 0.55,
    "SYMBOL_COOLDOWN_SEC": 900,
    "MAX_PENDING_PER_SYMBOL": 2,
    "MAX_NEW_SIGNALS_PER_CYCLE": 12,
    "MAX_NEW_SIGNALS_PER_HOUR": 80,
    "MAX_SAME_DIRECTION_RATIO": 0.85,
    "AUTO_HEAL_INTERVAL_SEC": 120,
    "MAX_EFFECTIVE_CONFIDENCE": 6.5,
    "SIGNAL_STARVATION_MINUTES": 10,
    "SIGNAL_STARVATION_THRESHOLD_MIN": 0.8,
    "SIGNAL_STARVATION_THRESHOLD_MAX": 1.5,
    "SIGNAL_STARVATION_RECOVERY_STEP": 0.10,
    "AUTO_HEAL_MIN_TRADES": 20,
    "ROLLBACK_IF_PF_DROP": 0.20,
    "ROLLBACK_IF_WR_DROP": 0.12,
    "RECENT_PERFORMANCE_WINDOW": 50,
    "DIRECTION_WINDOW": 80,
    "REGIME_WINDOW": 100,
...
class AdaptiveParameterOptimizer:
    ...
    def optimize(self, force: bool = False) -> dict:
        try:
            ...
            min_wf_test = CONFIG.get("MIN_WF_TEST_TRADES", 5)
            ...
            if "ATR_MULT_SL" in param_history and len(param_history["ATR_MULT_SL"]) > 3:
            ...
            if "EMA_SHORT" in param_history and len(param_history["EMA_SHORT"]) > 3:
            ...
            if "EMA_MED" in param_history and len(param_history["EMA_MED"]) > 3:
    ...
    def _optimize_risk_parameters(self) -> None:
        try:
            ...
            if winrate > 55 and profit_factor > 1.2 and sharpe > 0.5:
                multiplier = min(1.8, 1.0 + (winrate - 55) / 80 + (profit_factor - 1.2) / 4)
            elif winrate < 45 or profit_factor < 0.8:
            ...
            base_risk = CONFIG.get("RISK_PERCENT", 0.8)
            new_risk = base_risk * multiplier
            new_risk = max(CONFIG.get("MIN_RISK_PERCENT", 0.3), min(CONFIG.get("MAX_RISK_PERCENT", 2.0), new_risk))
    ...
    def optimize_regime_thresholds(self) -> None:
        try:
            min_samples = CONFIG.get("REGIME_MIN_TRADES_FOR_ADJUST", 15)
            max_adjustment = CONFIG.get("REGIME_MAX_ADJUSTMENT", 1.5)
            step = CONFIG.get("REGIME_ADJUSTMENT_STEP", 0.15)
...
class FeatureWeightLearner:
    def __init__(self, memory: PerformanceMemory):
        ...
        self.momentum: float = 0.40
        self.weight_changes: dict = {}
        self.correlation_threshold_positive: float = 0.10
        ...
        try:
            if os.path.exists(PATHS["FEATURE_WEIGHTS_FILE"]):
                with open(PATHS["FEATURE_WEIGHTS_FILE"], "r") as f:
                    saved = json.load(f)
                if isinstance(saved, dict) and saved.get("version") == 3 and isinstance(saved.get("weights"), dict):
                    ...
                    logger.debug(f"وزن‌های ویژگی نسخه 3 بارگذاری شد: {self.feature_weights}")
                else:
    ...
    def _save_weights(self):
        try:
            _atomic_write_json(PATHS["FEATURE_WEIGHTS_FILE"], {"version": 3, "weights": self.feature_weights})
    ...
    def update_weights(self) -> None:
        try:
            ...
            window = CONFIG.get("LEARNING_WINDOW", 80)
    ...
    def _get_current_sharpen_exponent(self) -> float:
        ...
        min_t = CONFIG.get("WEIGHT_SHARPEN_MIN_TRADES", 80)
        full_t = CONFIG.get("WEIGHT_SHARPEN_FULL_TRADES", 250)
        max_exp = min(CONFIG.get("WEIGHT_SHARPEN_EXPONENT_MAX", 1.8), 1.3)
        if n < min_t:
            return 1.0
        if n >= full_t:
            return max_exp
...
class MLConfidenceModel:
    ...
    def _load_model(self):
        try:
            ...
            logger.info(f"🤖 ML AutoPilot v4 loaded | {self.selected_model_name} | WF AUC={self.wf_auc_mean}")
        except Exception as e:
    ...
    def _build_dataset(self):
        ...
        for trade in self.memory.trades:
            ...
            if label not in ("TP1", "TP2", "TP3", "SL", "TIMEOUT"):
                continue
            # TIMEOUT considered as loss for more conservative training
    ...
    def should_retrain(self) -> bool:
        ...
        n = len(y)
        min_n = CONFIG.get("ML_MIN_TRADES_TO_TRAIN", 40)
    ...
    def _make_models():
        models = {
            "logistic": Pipeline([
                ...
                ("model", LogisticRegression(C=0.4, max_iter=1000, class_weight="balanced", random_state=42)),
            ]),
            "hist_gradient": Pipeline([
                ("impute", SimpleImputer(strategy="median", add_indicator=True)),
                ("model", HistGradientBoostingClassifier(
                    max_iter=120, max_leaf_nodes=8, learning_rate=0.04,
                    l2_regularization=1.5, min_samples_leaf=10,
        ...
        if CATBOOST_AVAILABLE:
            models["catboost"] = Pipeline([
                ("impute", SimpleImputer(strategy="median", add_indicator=True)),
                ("model", CatBoostClassifier(iterations=200, depth=4, learning_rate=0.04,
                    loss_function="Logloss", eval_metric="AUC", verbose=False, random_seed=42,
                    l2_leaf_reg=4.0, thread_count=1)),
            ])
        return models
    ...
    def _walk_forward(self, X, y):
        min_train = CONFIG.get("ML_MIN_TRAIN_SAMPLES", 40)
    ...
    def train(self, force: bool = False) -> Optional[dict]:
        ...
        try:
            ...
            block = CONFIG.get("ML_TEST_BLOCK", 15)
            gap = CONFIG.get("ML_WF_GAP", 5)
            test_start = max(CONFIG.get("ML_MIN_TRAIN_SAMPLES", 40), n - block)
            train_end = max(CONFIG.get("ML_MIN_TRAIN_SAMPLES", 40), test_start - gap)
            if train_end >= n:
                train_end = max(CONFIG.get("ML_MIN_TRAIN_SAMPLES", 40), n - block)
            ...
            active_std = min(0.15, float(CONFIG.get("ML_MAX_WF_AUC_STD", 0.20)))
            stable = (wf_auc >= CONFIG.get("ML_MIN_WF_AUC", 0.50) and
                      wf_std <= active_std and
                      (last_brier is None or last_brier <= CONFIG.get("ML_REQUIRE_BRIER_MAX", 0.30)))
            self.health = "ACTIVE" if stable else ("PROVISIONAL" if CONFIG.get("ML_ALLOW_PROVISIONAL_MODEL", True) else "OBSERVE_ONLY")
            self._save_model()

            logger.info(
                f"🤖 ML AutoPilot v4: {n} samples | {best_name} | "
    ...
    def get_influence_weight(self) -> float:
        ...
        min_auc = float(CONFIG.get("ML_MIN_AUC_FOR_INFLUENCE", 0.50))
        full_auc = float(CONFIG.get("ML_FULL_AUC_FOR_INFLUENCE", 0.60))
        ...
        if wf_auc < min_auc:
            return 0.0
        if brier > float(CONFIG.get("ML_REQUIRE_BRIER_MAX", 0.30)):
            return 0.0
        ...
        if self.health == "PROVISIONAL":
            ...
            return min(float(CONFIG.get("ML_MAX_INFLUENCE", 0.15)), base * max(0.4, quality) * max(0.5, stability) * holdout_factor)

        return min(float(CONFIG.get("ML_MAX_INFLUENCE", 0.15)),
                   maturity * quality * max(0.5, stability) * holdout_factor * float(CONFIG.get("ML_MAX_INFLUENCE", 0.15)))


# ============================================================================
# بخش 8: کلاس AutoPilot و Self-Healing - کامل
# ============================================================================
...
class SelfHealingAutoPilot:
    ...
    def regime_adjust(self):
        ...
        base = CONFIG["MIN_CONFIDENCE_SCORE"]
        for r, tr in regimes.items():
            ...
            if wr < CONFIG["REGIME_BAD_WINRATE"]:
                ...
                changed = True
            adj[r] = round(min(1.5, max(0.0, cur)), 2)
        if changed:
        ...
        with self.lock:
            ...
            if rm["pf"] < 0.85 or rm["wr"] < 0.44:
                ...
                if last_wr is not None and last_pf is not None and (
                    ...
                    CONFIG["MIN_CONFIDENCE_SCORE"] = min(6.2, CONFIG["MIN_CONFIDENCE_SCORE"] + 0.05)
                    CONFIG["PUMP_THRESHOLD"] = min(1.5, CONFIG["PUMP_THRESHOLD"] + 0.05)
                    CONFIG["DUMP_THRESHOLD"] = -CONFIG["PUMP_THRESHOLD"]
            elif rm["pf"] > 1.1 and rm["wr"] > 0.55:
                CONFIG["MIN_CONFIDENCE_SCORE"] = max(4.8, CONFIG["MIN_CONFIDENCE_SCORE"] - 0.05)
            self.regime_adjust()
...
# ============================================================================
# بخش 9: کلاس الگویاب (PatternRecognizer) - کامل
# ============================================================================
...
class PatternRecognizer:
    def __init__(self, memory: PerformanceMemory):
        ...
        self.similarity_threshold_success: float = 0.60
        self.similarity_threshold_fail: float = 0.50
...
# ============================================================================
# بخش 10: خودآموزی خودکار (Auto-Learning System) - کامل
# ============================================================================
            ...
            try:
                ...
                while len(self.learning_queue) > 0:
                    ...
                    if age >= hard_expiry_seconds:
                        ...
                        logger.error(f"🔴 {signal_data.get('symbol')} پس از {age/60:.0f} دقیقه (سقف مطلق) بدون پردازش از صف حذف شد")
                        continue
                    ...
                    if result and not result.get("_unresolved"):
                        ...
                        if len(self.memory.trades) % 3 == 0 and len(self.memory.trades) > 0:
                            ...
                            try:
                                ...
                                logger.error(f"خطا در به‌روزرسانی یادگیری (نادیده گرفته شد): {learn_err}")
                    else:
                        ...
                        if retry_count <= max_retries:
                            ...
                            logger.warning(f"❌ {signal_data.get('symbol')} پس از {max_retries} تلاش رد شد ({reason})")
...
# ============================================================================
# بخش 11: توابع کمکی (Utilities) - کامل
# ============================================================================
...
def get_max_hold_candles() -> int:
    minutes_per_candle = _interval_to_minutes(CONFIG.get("KLINE_INTERVAL", "15m")) or 15
    return max(1, round(CONFIG.get("MAX_HOLD_MINUTES", 60) / minutes_per_candle))
...
# ============================================================================
# بخش 12: مدیریت داده (Data Management) - کامل
# ============================================================================
...
def fetch_klines_df(symbol: str, interval: str = None, limit: int = None, use_cache: bool = True) -> Optional[pd.DataFrame]:
    try:
        ...
        if limit is None:
            limit = CONFIG.get("KLINE_LIMIT", 500)
...
# ============================================================================
# بخش 13: اندیکاتورهای تکنیکال (Technical Indicators) - کامل
# ============================================================================
...
# ============================================================================
# بخش 14: تشخیص الگوهای پرایس اکشن (Price Action) - کامل
# ============================================================================
...
# ============================================================================
# بخش 15: توابع سیگنال و بک‌تست - کامل
# ============================================================================
...
def compute_confirmation_signals(df: pd.DataFrame, require_volume: bool = True) -> pd.DataFrame:
    try:
        ...
        df["volume_spike"] = df["volume"] > (df["vol_sma20"] * 1.1)

        df["adx"] = adx_pd(df["high"], df["low"], df["close"])
        df["cci"] = cci_pd(df["high"], df["low"], df["close"], CONFIG.get("CCI_PERIOD", 15))
        ...
        trending_enough = df["adx"] >= CONFIG.get("ADX_TREND_MIN", 15)
        ...
        df["confluence_bull"] = bull_votes >= 2
        df["confluence_bear"] = bear_votes >= 2
...
def backtest_symbol_realistic(symbol: str, candles: int = None) -> dict:
    try:
        ...
        conf = compute_confirmation_signals(df, require_volume=CONFIG.get("REQUIRE_VOLUME_SPIKE", False))
...
def adaptive_parameters_from_df(df: pd.DataFrame) -> dict:
    try:
        ...
        if atr_ratio > CONFIG["VOLATILITY_REGIME_ATR_RATIO"]:
            params["ATR_MULT_SL"] = max(1.3, CONFIG["ATR_MULT_SL"] * 1.15)
        else:
...
# ============================================================================
# بخش 16: کامپوننت‌های سراسری یادگیری
# ============================================================================
...
def self_diagnose_and_repair() -> None:
    try:
        with CONFIG_LOCK:
            min_c = float(CONFIG.get("MIN_CONFIDENCE_SCORE", 5.0))
            max_c = float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.5))
            if max_c < 5.5:
                max_c = 5.5
            ...
            for regime in ("TRENDING", "VOLATILE", "RANGING", "UNKNOWN"):
                ...
                adj[regime] = round(max(0.0, min(1.5, v)), 2)
            CONFIG["REGIME_CONFIDENCE_ADJUSTMENT"] = adj

            if float(CONFIG.get("PUMP_THRESHOLD", 1.2)) < 0.6:
                CONFIG["PUMP_THRESHOLD"] = 0.6
            if float(CONFIG.get("PUMP_THRESHOLD", 1.2)) > 2.0:
...
# ============================================================================
# بخش 17: هسته تحلیل با یادگیری - کامل
# ============================================================================
...
def calculate_signal_confidence_with_learning(signal_data: dict, current_features: dict) -> float:
    try:
        ...
        if n_trades < min_t:
            ...
            learned_share = 0.20 + progress * (0.35 - 0.20)
...
def analyze_symbol_full(symbol: str, direction_hint: Optional[str] = None, use_multi_tf: bool = True) -> Optional[dict]:
    try:
        ...
        conf = compute_confirmation_signals(df, require_volume=False)
        ...
        volume_spike = bool(df["volume_ratio"].iloc[-1] > 1.1) if "volume_ratio" in df.columns else False
        df["adx"] = conf["adx"]
        df["cci"] = conf["cci"]
...
def enhanced_analysis(symbol: str, direction_hint: Optional[str] = None, recovery_mode: bool = False) -> Optional[dict]:
    try:
        ...
        hard_cap = float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.5))
        min_confidence = min(hard_cap,
                             float(CONFIG.get("MIN_CONFIDENCE_SCORE", 5.0)) +
                             float(CONFIG.get("REGIME_CONFIDENCE_ADJUSTMENT", {}).get(regime, 0)))
        if recovery_mode:
            min_confidence = min(min_confidence, float(CONFIG.get("FALLBACK_MIN_CONFIDENCE", 5.0)))
        
        idle_min = max(0.0, (time.time() - _LAST_ACCEPTED_SIGNAL_TS) / 60.0)
        starvation_start = float(CONFIG.get("SIGNAL_STARVATION_MINUTES", 10))
        if idle_min >= starvation_start:
            recovery_steps = int((idle_min - starvation_start) // 15) + 1
            recovery = min(0.70, recovery_steps * 0.10)
            min_confidence = max(4.5, min_confidence - recovery)
            min_confidence = min(min_confidence, float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.5)))
...
# ============================================================================
# بخش 18: توابع ذخیره‌سازی و گزارش
# ============================================================================
...
def create_ml_features(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        features["cci"] = cci_pd(df["high"], df["low"], df["close"], CONFIG.get("CCI_PERIOD", 15))
...
def build_learning_report(price_map: Dict[str, float]) -> str:
    ...
    try:
        ...
        regimes = summary.get("market_regimes", {})
        if regimes:
            lines.append("")
            lines.append("🌐 عملکرد به‌ازای رژیم بازار:")
            regime_adj = CONFIG.get("REGIME_CONFIDENCE_ADJUSTMENT", {})
            base_conf = CONFIG.get("MIN_CONFIDENCE_SCORE", 5.0)
            for regime, stats in regimes.items():
                adj = regime_adj.get(regime, 0)
                effective_threshold = min(float(CONFIG.get("MAX_EFFECTIVE_CONFIDENCE", 6.5)), float(base_conf + adj))
                threshold_note = f" | آستانه واقعی: {effective_threshold:.1f}"
        ...
        if SKLEARN_AVAILABLE:
            lines.append("")
            lines.append("🤖 مدل یادگیری ماشین (ML AutoPilot):")
            if ml_model.is_trained:
                ...
                lines.append(f"  • وضعیت: {getattr(ml_model, 'health', 'UNKNOWN')} | XGBoost: {XGBOOST_VERSION if XGBOOST_AVAILABLE else 'غیرفعال'}")
                ...
                min_auc = CONFIG.get("ML_MIN_AUC_FOR_INFLUENCE", 0.50)
                if influence_pct == 0 and ml_model.last_test_auc is not None and ml_model.last_test_auc < min_auc:
                    lines.append(f"    (صفر است چون AUC هنوز به {min_auc} نرسیده)")
            else:
                needed = CONFIG.get("ML_MIN_TRADES_TO_TRAIN", 40)
        ...
        if timing.get("available"):
            lines.append(f"  • میانگین حرکت قبل از سیگنال: {timing['avg_pre_signal_move_pct']:.2f}%")
            lines.append(f"  • احتمال رسیدن به TP1: {timing['tp1_hit_rate_pct']:.0f}%")
            lines.append(f"  • احتمال خوردن SL: {timing['sl_hit_rate_pct']:.0f}%")
            if timing.get("avg_time_to_result_min"):
                lines.append(f"  • میانگین زمان تا نتیجه: {timing['avg_time_to_result_min']:.0f} دقیقه")
            if timing.get("avg_mfe_on_losses_pct") is not None:
                lines.append(f"  • نزدیکی معاملات بازنده به TP1 قبل از برگشت: {timing['avg_mfe_on_losses_pct']:.2f}%")
        else:
            lines.append(f"  • داده کافی نیست هنوز ({timing.get('sample_size', 0)}/5 نمونه)")

        lines.append("")
        lines.append("⚖️ وزن فیچرها (چیزی که یاد گرفته):")
        for feat, weight in sorted(weight_learner.feature_weights.items(), key=lambda x: -x[1]):
            prev = _LAST_REPORTED_WEIGHTS.get(feat, weight)
            delta = weight - prev
            arrow = "📈" if delta > 0.01 else "📉" if delta < -0.01 else "➖"
            lines.append(f"  {arrow} {feat}: {weight:.2f}" + (f" ({delta:+.2f})" if abs(delta) > 0.01 else ""))
        _LAST_REPORTED_WEIGHTS = weight_learner.feature_weights.copy()

        lines.append("")
        lines.append("🎯 الگوهای یاد گرفته‌شده:")
        lines.append(f"  • الگوهای موفق: {len(pattern_recognizer.successful_patterns)}")
        lines.append(f"  • الگوهای ناموفق: {len(pattern_recognizer.failed_patterns)}")

        lines.append("")
        lines.append(f"⏳ سیگنال‌های در صف انتظار (هنوز نهایی نشده): {pending['total_pending']}")
        if pending["total_pending"] > 0:
            lines.append(f"  • رسیده به TP1: {pending['tp1_hit']}")
            lines.append(f"  • خورده به SL: {pending['sl_hit']}")
        ...
        logger.error(f"خطا در ساخت گزارش یادگیری: {e}")
        return "⚠️ خطا در ساخت گزارش یادگیری"


# ============================================================================
# بخش 19: حلقه اصلی (Main Loop) - کامل
# ============================================================================
...
def get_adaptive_trigger_thresholds() -> tuple:
    try:
        ...
        if idle_min >= CONFIG.get("SIGNAL_STARVATION_MINUTES", 10):
            steps = int((idle_min - CONFIG.get("SIGNAL_STARVATION_MINUTES", 10)) // 20) + 1
            base -= steps * CONFIG.get("SIGNAL_STARVATION_RECOVERY_STEP", 0.10)
            base = max(CONFIG.get("SIGNAL_STARVATION_THRESHOLD_MIN", 0.8), base)
        base = min(CONFIG.get("SIGNAL_STARVATION_THRESHOLD_MAX", 1.5), base)
        return base, -base
    except Exception:
...
def main_loop():
    ...
    logger.info(f"🛟 Signal recovery: starts={CONFIG.get('SIGNAL_STARVATION_MINUTES',10)}m | max_conf={CONFIG.get('MAX_EFFECTIVE_CONFIDENCE',6.5)}")
        ...
        try:
            ...
            for coin in coins:
                ...
                if not allowed:
                    ...
                    continue
                if len(signals) >= CONFIG.get("MAX_NEW_SIGNALS_PER_CYCLE", 12):
                    continue
            ...
            idle_min = max(0.0, (time.time() - _LAST_ACCEPTED_SIGNAL_TS) / 60.0)
            fallback_after = float(CONFIG.get("FALLBACK_SCAN_AFTER_MINUTES", 8))
            if len(signals) < 2 and idle_min >= fallback_after:
                ...
                candidates = candidates[:int(CONFIG.get("FALLBACK_CANDIDATES", 20))]
                logger.info(f"🛟 Smart fallback فعال شد | idle={idle_min:.1f}m | candidates={len(candidates)}")
                for _, ch, symbol in candidates:
                    if len(signals) >= int(CONFIG.get("MAX_NEW_SIGNALS_PER_CYCLE", 12)):
                        break
            ...
            if now_hour - last_learning_update >= 12:
...
# ============================================================================
# بخش 20: نقطه شروع برنامه
# ============================================================================

if __name__ == "__main__":
    print("=" * 60)
    print("🚀 سیستم تحلیل سیگنال نسخه 14.0 AUTO-PILOT - Self-Healing + Smart Signal Recovery")
