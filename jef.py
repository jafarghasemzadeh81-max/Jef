...
CONFIG = {
    ...
    # فقط وقتی influence واقعی ML از این آستانه بالاتر باشد، weak_ml اعمال می‌شود.
    "PRO_ML_GATE_MIN_INFLUENCE": 0.02,
    ...
    # کارت GIF متحرک برای سیگنال (نیاز به Pillow)
    "SIGNAL_GIF_ENABLED": True,
    "SIGNAL_GIF_FRAMES": 16,
    "SIGNAL_GIF_FRAME_MS": 90,
...
class AdaptiveParameterOptimizer:
    ...
    def optimize(self, force: bool = False) -> dict:
        try:
            ...
            # تنظیم MIN_CONFIDENCE_SCORE (گام کوچک برای جلوگیری از نوسان با starvation)
...
# ---------------------------------------------------------------------------
# کارت سیگنال GIF (پالس) — بدون Timeframe
# ---------------------------------------------------------------------------
...
def render_signal_gif(summary: dict, out_path: Optional[str] = None) -> Optional[str]:
    """ساخت GIF پالس‌دار کارت سیگنال. در صورت نبود Pillow → None."""
    if not PIL_AVAILABLE:
        return None
    try:
        W, H = 540, 880
        n_frames = int(CONFIG.get("SIGNAL_GIF_FRAMES", 16))
        frame_ms = int(CONFIG.get("SIGNAL_GIF_FRAME_MS", 90))
        ...
        def pulse(t, lo=0.35, hi=1.0):
            s = 0.5 - 0.5 * math.cos(2 * math.pi * t)
            return lo + (hi - lo) * s

        f_title = _signal_gif_font(42, True)
        f_sym = _signal_gif_font(48, True)
        f_label = _signal_gif_font(26)
        f_val = _signal_gif_font(28, True)
        ...
        for i in range(n_frames):
            ...
            if is_long:
                outline = (0, lerp(40, 180, p3), lerp(60, 120, p3))
            else:
                outline = (lerp(40, 200, p3), lerp(30, 80, p3), lerp(30, 80, p3))
            d.rounded_rectangle([36, 40, W - 36, H - 40], radius=36, fill=(18, 22, 28), outline=outline, width=3)

            if is_long:
                badge_col = (0, lerp(120, 255, p), lerp(80, 180, p))
                badge_txt = (5, 20, 10)
            else:
                badge_col = (lerp(140, 255, p), lerp(40, 90, p), lerp(40, 90, p))
                badge_txt = (30, 5, 5)
            d.rounded_rectangle([W // 2 - 80, 70, W // 2 + 80, 120], radius=24, fill=badge_col)
            tw = d.textlength(direction, font=f_title)
            d.text(((W - tw) / 2, 78), direction, font=f_title, fill=badge_txt)

            tw = d.textlength(symbol, font=f_sym)
            d.text(((W - tw) / 2, 145), symbol, font=f_sym, fill=(240, 244, 250))
            if is_long:
                div = (0, lerp(60, 160, p2), lerp(40, 100, p2))
            else:
                div = (lerp(80, 180, p2), lerp(40, 80, p2), lerp(40, 80, p2))
            d.line([70, 210, W - 70, 210], fill=div, width=2)
            ...
            if is_long:
                rows = [
                    ("Entry", entry_s, (100, 180, 255), (240, 244, 250), p2),
                    ("Stop Loss", sl_s, (lerp(150, 255, p), 70, 70), (lerp(180, 255, p), 90, 90), p),
                    ("TP1", tp_s[0], (0, lerp(140, 255, p), lerp(100, 180, p)), (0, lerp(160, 255, p), lerp(120, 200, p)), p),
                    ("TP2", tp_s[1], (0, lerp(120, 220, p2), lerp(90, 160, p2)), (0, lerp(150, 240, p2), lerp(110, 180, p2)), p2),
                    ("TP3", tp_s[2], (0, lerp(100, 200, p3), lerp(80, 150, p3)), (0, lerp(140, 230, p3), lerp(100, 170, p3)), p3),
                ]
            else:
                rows = [
                    ("Entry", entry_s, (100, 180, 255), (240, 244, 250), p2),
                    ("Stop Loss", sl_s, (70, lerp(150, 255, p), 90), (90, lerp(180, 255, p), 110), p),
                    ("TP1", tp_s[0], (lerp(140, 255, p), 70, 70), (lerp(180, 255, p), 90, 90), p),
                    ("TP2", tp_s[1], (lerp(120, 230, p2), 60, 60), (lerp(160, 240, p2), 80, 80), p2),
                    ("TP3", tp_s[2], (lerp(100, 200, p3), 55, 55), (lerp(140, 220, p3), 75, 75), p3),
                ]

            y = 240
            for label, val, icon_col, val_col, phase in rows:
                d.rounded_rectangle([60, y - 8, W - 60, y + 52], radius=16, fill=(24, 28, 36))
                r, cx, cy = 14, 92, y + 22
                for g in range(3, 0, -1):
                    col = tuple(max(0, min(255, int(c * (0.25 + 0.3 * g * phase)))) for c in icon_col)
                    d.ellipse([cx - r - g * 3, cy - r - g * 3, cx + r + g * 3, cy + r + g * 3], outline=col, width=2)
                d.ellipse([cx - r, cy - r, cx + r, cy + r], fill=icon_col)
                d.text((130, y + 6), label, font=f_label, fill=(150, 160, 175))
                tw = d.textlength(val, font=f_val)
                d.text((W - 80 - tw, y + 5), val, font=f_val, fill=val_col)
                y += 78

            d.line([70, y + 10, W - 70, y + 10], fill=(40, 50, 60), width=1)
            y += 30
            d.text((70, y), "Confidence", font=f_label, fill=(150, 160, 175))
            conf_s = f"{conf:.1f}/10"
            tw = d.textlength(conf_s, font=f_val)
            conf_col = (0, lerp(180, 255, p3), lerp(140, 200, p3)) if is_long else (lerp(180, 255, p3), lerp(100, 160, p3), lerp(100, 160, p3))
            d.text((W - 70 - tw, y), conf_s, font=f_val, fill=conf_col)
            bar_y = y + 42
            d.rounded_rectangle([70, bar_y, W - 70, bar_y + 18], radius=9, fill=(30, 36, 46))
            fill_w = int((W - 140) * (conf / 10.0))
            bar_col = (0, lerp(140, 255, p3), lerp(100, 180, p3)) if is_long else (lerp(140, 255, p3), lerp(60, 120, p3), lerp(60, 120, p3))
            if fill_w > 0:
                d.rounded_rectangle([70, bar_y, 70 + fill_w, bar_y + 18], radius=9, fill=bar_col)
                shim_x = 70 + int(max(0, fill_w - 30) * t)
                d.rectangle(
                    [shim_x, bar_y + 2, min(70 + fill_w, shim_x + 24), bar_y + 16],
                    fill=(180, 255, 220) if is_long else (255, 200, 200),
                )
            y = bar_y + 50
            d.text((70, y), "Regime", font=f_label, fill=(150, 160, 175))
            tw = d.textlength(regime, font=f_val)
            reg_col = (0, lerp(160, 255, p2), lerp(120, 190, p2)) if is_long else (lerp(160, 255, p2), lerp(100, 160, p2), lerp(100, 160, p2))
            d.text((W - 70 - tw, y), regime, font=f_val, fill=reg_col)
            frames.append(img)
...
def deliver_signal_to_telegram(summary: dict, source: str = "normal") -> str:
    ...
    try:
        ...
        sent = False
        gif_path = None
        use_gif = bool(CONFIG.get("SIGNAL_GIF_ENABLED", True)) and PIL_AVAILABLE
        if use_gif:
            try:
                gif_path = render_signal_gif(summary)
                if gif_path:
                    # فقط GIF — بدون نوشته زیر کارت
                    sent = send_telegram_animation(gif_path, caption="")
            except Exception as gif_err:
                logger.warning(f"GIF signal failed, fallback to text: {gif_err}")
                sent = False
            finally:
                if gif_path:
                    try:
        ...
        if sent:
            ...
            logger.info(f"📨 Telegram delivery | source={source} | {symbol} {direction} | SENT{' (GIF)' if use_gif and PIL_AVAILABLE else ''}")
            return "SENT"
...
def calculate_signal_confidence_with_learning(signal_data: dict, current_features: dict) -> float:
    try:
        ...
        if actual_ml_influence > 0.005 and ml_model is not None and ml_model.is_trained:
...
def pro_signal_quality_gate(signal: dict) -> Tuple[bool, str, dict]:
    ...
    try:
        ...
        min_infl_for_gate = float(CONFIG.get("PRO_ML_GATE_MIN_INFLUENCE", 0.02))
        if (
            signal.get("ml_used")
            ...
            diagnostics["ml_gate_skipped"] = f"health={ml_health}|infl={actual_ml_influence:.4f}"
...
def _resolve_signals_in_window(since_ts: float, until_ts: float) -> dict:
    ...
    for sig in unique_sigs:
        ...
        if trade is None:
            n_open += 1
            rows.append({"emoji": "⏳", "sym": sym, "direction": direction, "tag": "OPEN", "ret_s": ""})
            continue
...
def send_6h_outcome_summary_safely() -> bool:
    ...
    try:
        ...
        text = build_6h_outcome_summary(since_ts=since if since > 0 else None)
        chunks = _split_telegram_text(text, CONFIG.get("REPORT_MAX_TELEGRAM_CHARS", 3800))
        if not chunks:
            return False
        ok = True
        for i, chunk in enumerate(chunks, 1):
            ...
            if not send_telegram_message(prefix + chunk, message_type="6h-outcome summary", channel="report"):
                ok = False
        if ok:
            _OUTCOME_STATE["last_6h_ts"] = time.time()
            _save_outcome_report_state()
        return ok
    except Exception as e:
