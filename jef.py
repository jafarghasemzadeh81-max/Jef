...
def build_complete_report(price_map: Dict[str, float]) -> str:
    ...
    try:
        ...
        lines = []
        lines.append("=" * 60)
        lines.append("📊 **گزارش جامع ۳۰ دقیقه‌ای - نسخه 21.4 FINAL DELIVERY**")
        ...
        try:
            ...
            for coin in coins:
                ...
                if summary:
                    ...
                    deliver_signal_to_telegram(summary, source="normal")
            ...
            if len(signals) < 2 and idle_min >= fallback_after:
                ...
                for _, ch, symbol in candidates:
                    ...
                    deliver_signal_to_telegram(summary, source="fallback")
            ...
            if time.time() - last_report_time >= 1800:  # هر ۳۰ دقیقه
                ...
                # گزارش هم از همان منبع مرکزی تنظیمات استفاده می‌کند.
                ...
                if report_sent:
                    ...
                    last_report_time = time.time()  # جلوگیری از تلاش مکرر در هر چرخه
